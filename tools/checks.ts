...
group('A level and a star in one helping are two cards, in that order', () => {
  ...
  check(
    'and so does the day',
    nextCelebration({ ...queue, levelUp: false, starUp: false, hold: 1.4 }) === null,
  );
  check(
  ...
  check(
    'an empty queue shows nothing',
    nextCelebration({ ...queue, levelUp: false, starUp: false, dayRecap: false }) === null,
  );
